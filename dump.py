...
def Dump(channel_id):
    ...
    url = str( rtmp2m3u8(dbutil.get_live_url(channel_id) ))  #'http://1.8.23.98/live/stream9.m3u8'  #"http://1.8.203.198/live/stream1.m3u8"
        ...
        while True:
            if Dump.cur_index not in ts_map:
                ...
                if cur_time - start_time >= timedelta(minutes=1):
                    ...
                    logger.debug('ts file create --' + ts_path + name +'-' + '{:.3f}'.format(ts_duration) +'.ts   ' + str(start_time))
                    ts_duration = 0.0
